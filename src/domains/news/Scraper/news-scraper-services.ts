...
class NewsScraperServices {
   ...
   public scrapeGoogleNews_v2 = async (searchKey: string): Promise<void> => {
      ...
      try {
        const filePath = `./google_news_result_${searchKey.replace(/\s+/g, '_')}.json`; // optional: bikin dinamis berdasarkan keyword
        fs.writeFileSync(filePath, JSON.stringify(formattedResponse, null, 2), 'utf-8');
        console.info(`[File Export] News saved to ${filePath}`);
      } catch (e: any) {
        console.error(`[File Write Error]`, e);
      }
    
      // Proses simpan ke MongoDB
      try {
        const collection = await connectMongo();
        const result = await collection.insertMany(formattedResponse);
